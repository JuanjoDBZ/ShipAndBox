//
//  SABRecoverPasswordRemoteData.swift
//  ShipAndBox
//
//  Created by ISITA on 01/07/21.
//
import Foundation
class SABRecoverPasswordRemoteData:NSObject,URLSessionDelegate {
    /// Función para consultar el ws
    /// - Parameter parametersCreateSignature: Se envia el correo del usuario
    func sendSignatureUserSABRemoteData<T: Decodable>(parameters: NSDictionary, objectType: T.Type, completion: @escaping (EnumsRequestAndErrors.Result<T>) -> Void) {
        /// path para complemetar la url
        let path = "sendEmailRecoveryPassword"
